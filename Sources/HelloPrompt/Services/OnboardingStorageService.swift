...
public class OnboardingStorageService: ObservableObject {
    ...
    public func skipOnboarding(permanent: Bool = false) {
        ...
        if let analytics = currentAnalytics {
            ...
            let updatedAnalytics = OnboardingAnalytics(
                sessionId: analytics.sessionId,
                startedAt: analytics.startedAt,
                ...
                timePerStep: analytics.timePerStep,
                userInteractions: analytics.userInteractions + [interaction],
                errorOccurred: analytics.errorOccurred
            )
    ...
    public func recordError(_ error: OnboardingError) {
        ...
        let updatedAnalytics = OnboardingAnalytics(
            ...
            timePerStep: analytics.timePerStep,
            userInteractions: analytics.userInteractions,
            errorOccurred: analytics.errorOccurred + [error]
        )
        currentAnalytics = updatedAnalytics
        saveAnalytics()
    ...
    public func updateStepTime(_ step: OnboardingStepType, timeSpent: TimeInterval) {
        ...
        var updatedTimePerStep = analytics.timePerStep
        updatedTimePerStep[step] = timeSpent
        ...
        let updatedAnalytics = OnboardingAnalytics(
            ...
            stepsSkipped: analytics.stepsSkipped,
            totalTimeSpent: analytics.totalTimeSpent,
            timePerStep: updatedTimePerStep,
            userInteractions: analytics.userInteractions,
            errorOccurred: analytics.errorOccurred
        )
        currentAnalytics = updatedAnalytics
    ...
    // MARK: - Additional Methods for OnboardingCoordinator compatibility
    
    /// Check if onboarding has been permanently skipped
    public var isOnboardingSkippedPermanently: Bool {
        return userDefaults.bool(forKey: StorageKeys.skipOnboarding)
    }
    
    /// Check if onboarding should restart
    public func shouldRestartOnboarding() -> Bool {
        return hasOnboardingVersionChanged() || isFirstRun()
    }
    
    /// Reset the onboarding skip state
    public func resetOnboardingSkipState() {
        userDefaults.removeObject(forKey: StorageKeys.skipOnboarding)
        logManager.info("OnboardingStorageService", "Onboarding skip state reset")
    }
    
    /// Save onboarding progress
    public func saveOnboardingProgress(_ progress: OnboardingProgress) {
        updateProgress(progress)
    }
    
    /// Get saved progress
    public func getSavedProgress() -> OnboardingProgress? {
