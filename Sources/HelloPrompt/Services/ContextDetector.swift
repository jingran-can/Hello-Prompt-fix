...
public final class ContextDetector: ObservableObject {
    ...
    // MARK: - 初始化
    public init() {
        // 修复双重弹窗：初始化时不自动请求权限，避免与EnhancedPermissionManager冲突
    ...
    private func checkAccessibilityPermissionSilently() {
        ...
        // 静默版本：只记录调试信息，不记录警告避免日志噪音
        LogManager.shared.debug("ContextDetector", "静默权限检查: 辅助功能权限状态=\(accessibilityPermissionGranted ? "已授权" : "未授权")")
    }
    
    public func requestAccessibilityPermission() -> Bool {
        LogManager.shared.info("ContextDetector", "静默检查辅助功能权限")
        
        // 修复双重弹窗：只使用静默检查，不触发系统弹窗
        let currentStatus = AXIsProcessTrusted()
        accessibilityPermissionGranted = currentStatus
        hasAccessibilityPermission = currentStatus
        
        if currentStatus {
            LogManager.shared.info("ContextDetector", "✅ 辅助功能权限已授权")
        } else {
            LogManager.shared.info("ContextDetector", "⚠️ 辅助功能权限未授权")
        }
    ...
    public func startMonitoring() {
        ...
        if !accessibilityPermissionGranted {
            ...
            if currentStatus {
                ...
                LogManager.shared.info("ContextDetector", "✅ 检测到辅助功能权限已授权，直接启动监控")
            } else {
                // 权限未授权，但不主动请求（避免与EnhancedPermissionManager冲突）
                LogManager.shared.info("ContextDetector", "⚠️ 辅助功能权限未授权，监控功能受限")
            }
        }
