...
final class OptionKeyService: ObservableObject {
    ...
    private var eventTap: CFMachPort? {
        willSet { if let old = eventTap { CFRelease(old) } }
    }
    private var runLoopSource: CFRunLoopSource? {
        willSet { if let old = runLoopSource {
            CFRunLoopRemoveSource(CFRunLoopGetCurrent(), old, .commonModes)
        }}
    }
    ...
    func requestAccessibilityPermission() {
        // 暂时禁用权限弹窗请求以避免循环
        LogManager.shared.info("OptionKeyService", "辅助功能权限弹窗请求已暂时禁用以避免循环")
        return
