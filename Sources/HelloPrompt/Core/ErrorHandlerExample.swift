...
public class ExampleAudioService: ObservableObject {
    ...
    public func startRecording() async {
        ...
        do {
            ...
            // 处理自定义错误
            errorHandler.handle(error, context: "ExampleAudioService.startRecording")
        } catch {
            // 处理系统错误
            let wrappedError = AudioSystemError.audioEngineFailure(error)
            errorHandler.handle(wrappedError, context: "ExampleAudioService.startRecording")
        }
    }
    ...
    public func stopRecording() async {
        ...
        do {
            ...
            let wrappedError = AudioSystemError.audioEngineFailure(error)
            errorHandler.handle(wrappedError, context: "ExampleAudioService.stopRecording")
        }
    }
    ...
    public func playAudio(url: URL) async {
        ...
        do {
            ...
            errorHandler.handle(error, context: "ExampleAudioService.playAudio")
        } catch {
            let wrappedError = AudioSystemError.audioEngineFailure(error)
            errorHandler.handle(wrappedError, context: "ExampleAudioService.playAudio")
        }
    }
    ...
    public func loadData() {
        Task {
            ...
            do {
                ...
                errorHandler.handle(uiError, context: "ViewController.loadData")
