...
public class OnboardingCoordinator: ObservableObject {
    ...
    public func skipOnboarding(permanent: Bool = false) {
        ...
        if permanent {
            ...
            if let progress = flowManager.progress {
    ...
    public func handleError(_ error: OnboardingError) {
        ...
        trackOnboardingEvent("onboarding_error", properties: [
            "error_code": error.errorType.rawValue,
        ...
        flowManager.$currentError
            ...
            .sink { [weak self] error in
                let onboardingError = OnboardingError(
                    step: self?.flowManager.currentStep ?? .startupIntroduction,
                    errorType: .unknownError,
                    message: error.localizedDescription,
                    underlyingError: "\(error)"
                )
                self?.handleError(onboardingError)
            }
            .store(in: &cancellables)
    ...
    private func checkInitialState() {
        ...
        if let savedProgress = storageService.getSavedProgress(),
           !storageService.isOnboardingCompleted {
            ...
            Task {
                do {
                    try await flowManager.resumeFromProgress(savedProgress)
                    await MainActor.run {
            updatePhaseFromStep(flowManager.currentStep)
            updateCompletionPercentage(savedProgress)
                    }
                } catch {
                    LogManager.shared.error("OnboardingCoordinator", "Failed to resume from progress: \(error)")
                }
            }
        }
    ...
    private func calculateOptionalStepsCompleted() -> Int {
        ...
        if !configManager.selectedModelId.isEmpty {
            completed += 1
        }
    ...
    private func attemptErrorRecovery(_ error: OnboardingError) {
        LogManager.shared.info("OnboardingCoordinator", "Attempting error recovery for: \(error.errorType.rawValue)")
        
        switch error.errorType {
        case .permissionDenied:
            // For permission errors, we can guide user to system preferences
            LogManager.shared.info("OnboardingCoordinator", "Permission denied recovery")
            
        case .invalidApiKey:
            // For API key errors, we can suggest re-entering the key
            LogManager.shared.info("OnboardingCoordinator", "Invalid API key recovery")
            
        case .networkError:
            // For network errors, we can suggest checking connection
            LogManager.shared.info("OnboardingCoordinator", "Network error recovery")
            
        default:
            // For other errors, show generic recovery options
            LogManager.shared.info("OnboardingCoordinator", "Generic error recovery")
        }
    }
