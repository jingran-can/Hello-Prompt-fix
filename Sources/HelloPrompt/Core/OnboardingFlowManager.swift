...
public class OnboardingFlowManager: ObservableObject {
    ...
    public func resumeFromProgress(_ progress: OnboardingProgress) async throws {
        currentStep = progress.currentStep
        logManager.info("OnboardingFlowManager", "Resuming onboarding from step: \(progress.currentStep.title)")
        
        // Update UI state if needed
        await MainActor.run {
            // Force UI update for the resumed step
    ...
    public func testModel(_ modelId: String) async {
        ...
        do {
            ...
            // Test the model
            let result = await openAIService.optimizePrompt(testPrompt)
            let endTime = Date()
            ...
            case .success(let optimizationResult):
                testResults = ModelTestResults(
                    ...
                    tokensUsed: optimizationResult.optimizedText.count / 4, // Rough estimate
                    cost: nil, // Could calculate if pricing info available
                    testPrompt: testPrompt,
                    testResponse: optimizationResult.optimizedText,
                    testedAt: endTime
                )
            case .failure(let error):
