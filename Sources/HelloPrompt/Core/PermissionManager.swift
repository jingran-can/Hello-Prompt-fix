...
public struct PermissionState: Equatable {
    ...
    public static func == (lhs: PermissionState, rhs: PermissionState) -> Bool {
...
public final class PermissionManager: ObservableObject {
    ...
    // MARK: - 全局权限检查控制
    public static var isPermissionCheckingGloballyDisabled = false
    
    /// 全局禁用权限检查（彻底避免循环）
    public static func disableAllPermissionChecks() {
        isPermissionCheckingGloballyDisabled = true
        LogManager.shared.info("PermissionManager", "🚫 全局禁用所有权限检查以避免循环")
    }
    
    /// 全局启用权限检查
    public static func enableAllPermissionChecks() {
        isPermissionCheckingGloballyDisabled = false
        LogManager.shared.info("PermissionManager", "✅ 全局启用权限检查")
    }
    ...
    public func startPermissionConfigurationFlow() {
        ...
        LogManager.shared.info("PermissionManager", "🔧 开始权限配置流程，暂停自动检查")
    }
    
    /// 结束权限配置流程
    public func endPermissionConfigurationFlow() {
        isInPermissionConfigurationFlow = false
        LogManager.shared.info("PermissionManager", "✅ 权限配置流程结束，恢复正常检查")
    }
    ...
    private func setupApplicationStateObserver() {
        ...
        NotificationCenter.default.publisher(for: NSApplication.didResignActiveNotification)
            .sink { [weak self] _ in
                Task { @MainActor in
                    // 应用失去焦点时可以进行一些清理工作
                    LogManager.shared.debug("PermissionManager", "应用失去焦点")
                    // 如果是因为打开系统设置失去焦点，开始权限配置流程
                    self?.startPermissionConfigurationFlow()
                }
            }
            .store(in: &cancellables)
        ...
        LogManager.shared.info("PermissionManager", "应用状态监听器已设置（智能模式）")
    }
    ...
    private func setupPeriodicCheck() {
        ...
        //     }
        // }
        LogManager.shared.info("PermissionManager", "⏸️ 定期权限检查定时器已禁用，避免双重弹窗")
    }
    ...
    public func checkAllPermissions(reason: String = "手动检查") async {
        // 全局权限检查禁用检查
        if Self.isPermissionCheckingGloballyDisabled {
            LogManager.shared.info("PermissionManager", "🚫 权限检查已全局禁用，跳过检查 - 原因: \(reason)")
            return
        }
        
        // 如果正在权限配置过程中，跳过自动检查
        if isInPermissionConfigurationFlow {
            LogManager.shared.info("PermissionManager", "🔧 权限配置流程中，跳过自动检查 - 原因: \(reason)")
            return
        }
    ...
    public func checkPermission(_ type: PermissionType) async {
        // 全局权限检查禁用检查
        if Self.isPermissionCheckingGloballyDisabled {
            LogManager.shared.debug("PermissionManager", "🚫 权限检查已全局禁用，跳过单个权限检查 - 类型: \(type.rawValue)")
            return
        }
    ...
    private func forceRefreshCorePermissions() async {
        // 全局权限检查禁用检查
        if Self.isPermissionCheckingGloballyDisabled {
            LogManager.shared.debug("PermissionManager", "🚫 权限检查已全局禁用，跳过强制刷新核心权限")
            return
        }
        ...
        // 不再清除麦克风权限缓存，避免死循环
        // 由于缓存被清除为notDetermined，然后checkPermission会检测到变为Granted
        // 这导致了"权限状态变化：microphone Not Determined -> Granted"循环
        LogManager.shared.info("PermissionManager", "⚠️ 防止权限循环：保留麦克风权限缓存")
    ...
    private func getNotificationPermissionStatus() -> PermissionStatus {
        ...
        LogManager.shared.debug("PermissionManager", "使用Bundle ID进行通知权限检查: \(bundleId)")
    ...
    private func getNotificationPermissionStatusAsync() async -> PermissionStatus {
        // 使用HelloPromptApp_Integrated的bundleIdentifier fallback
        let bundleId = HelloPromptApp_Integrated.bundleIdentifier
        LogManager.shared.debug("PermissionManager", "使用Bundle ID进行异步通知权限检查: \(bundleId)")
    ...
    private func getNotificationCenterSafely() -> UNUserNotificationCenter? {
        // 使用HelloPromptApp_Integrated的bundleIdentifier fallback
        let bundleId = HelloPromptApp_Integrated.bundleIdentifier
        LogManager.shared.debug("PermissionManager", "尝试获取通知中心，Bundle ID: \(bundleId)")
        
        // 检查是否是完整的 .app bundle
        let bundleURL = Bundle.main.bundleURL
        guard bundleURL.pathExtension == "app" else {
            LogManager.shared.warning("PermissionManager", "⚠️  非标准app bundle结构，跳过UNUserNotificationCenter")
            LogManager.shared.info("PermissionManager", "当前Bundle URL: \(bundleURL.path)")
            return nil
        }
    ...
    public func requestPermission(_ type: PermissionType) async -> PermissionStatus {
        ...
        if !canRequestPermission(type) {
            ...
            return await getPermissionStatus(type)
        }
    ...
    private func requestNotificationPermissionModern() async -> PermissionStatus {
        // 使用HelloPromptApp_Integrated的bundleIdentifier fallback
        let bundleId = HelloPromptApp_Integrated.bundleIdentifier
        LogManager.shared.debug("PermissionManager", "使用Bundle ID进行通知权限请求: \(bundleId)")
    ...
    private func startWaitingForAccessibilityPermission() {
        // 暂时禁用等待权限的定时器以避免循环
        LogManager.shared.info("PermissionManager", "等待辅助功能权限的定时器已暂时禁用")
        return
    ...
    private func requestNotificationPermission() async -> Bool {
        // 使用HelloPromptApp_Integrated的bundleIdentifier fallback
        let bundleId = HelloPromptApp_Integrated.bundleIdentifier
        LogManager.shared.debug("PermissionManager", "使用Bundle ID进行通知权限申请: \(bundleId)")
    ...
    public func forceRefreshAccessibilityPermission() async {
        // 全局权限检查禁用检查
        if Self.isPermissionCheckingGloballyDisabled {
            LogManager.shared.debug("PermissionManager", "🚫 权限检查已全局禁用，跳过强制刷新辅助功能权限")
            return
        }
        
        let realTimeStatus = checkAccessibilityPermissionRealTime()
        let currentStoredStatus = hasPermission(.accessibility)
        
        if realTimeStatus != currentStoredStatus {
            ...
            let oldStatus: PermissionStatus = currentStoredStatus ? .granted : .denied
    ...
    public func forceRefreshPermissions() async {
        // 全局权限检查禁用检查
        if Self.isPermissionCheckingGloballyDisabled {
            LogManager.shared.debug("PermissionManager", "🚫 权限检查已全局禁用，跳过强制刷新所有权限")
            return
        }
    ...
    public func immediatePermissionCheck() async {
        // 全局权限检查禁用检查
        if Self.isPermissionCheckingGloballyDisabled {
            LogManager.shared.debug("PermissionManager", "🚫 权限检查已全局禁用，跳过立即权限检查")
            return
        }
        ...
        // 直接重新检查所有权限，不重置状态
        await checkAllPermissions(reason: "立即权限检查")
    ...
    public func startPermissionMonitoring() {
        // 暂时禁用权限监控定时器以避免循环问题
        LogManager.shared.info("PermissionManager", "权限监控定时器已暂时禁用以避免循环")
        return
