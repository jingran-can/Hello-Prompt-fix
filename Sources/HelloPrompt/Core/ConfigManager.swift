...
public class AppConfigManager: ObservableObject {
    ...
    @Published var selectedModelId: String = "gpt-4o-mini"
    ...
    private func loadEnvironmentVariables() {
        print("[ConfigManager] 🔍 开始查找 .env 文件...")
        ...
        let possiblePaths = [
            ...
            // 2. 项目根目录
            "\(FileManager.default.currentDirectoryPath)/v2/.env",
            // 3. Bundle资源目录
            Bundle.main.path(forResource: ".env", ofType: nil),
            // 4. Bundle的Resources子目录
            Bundle.main.resourcePath.map { "\($0)/.env" },
            Bundle.main.resourcePath.map { "\($0)/Resources/.env" }
        ].compactMap { $0 }
        
        // 简化路径检查，只在找到时记录
        guard let envPath = possiblePaths.first(where: { FileManager.default.fileExists(atPath: $0) }) else {
            print("[ConfigManager] 🔧 使用内置配置（未找到.env文件）")
            // 尝试直接设置环境变量作为备选方案
            setEnvironmentVariable("OPENAI_API_KEY", value: "sk-Ejg7sfczFCWwC7vo503540Ca7a3d45Ec9d2d2f003d2f35Af")
            setEnvironmentVariable("OPENAI_BASE_URL", value: "https://api.openai.com/v1")
            setEnvironmentVariable("OPENAI_MODEL", value: "gpt-3.5-turbo")
            print("[ConfigManager] ✅ 环境变量已配置")
            return
        }
        
        print("[ConfigManager] ✅ 找到 .env 文件: \(envPath)")
        
        guard let envContent = try? String(contentsOfFile: envPath) else {
            print("[ConfigManager] ❌ 无法读取 .env 文件内容")
            return
        }
        
        print("[ConfigManager] 📖 .env 文件内容:")
        ...
        for (lineNum, line) in lines.enumerated() {
            ...
            print("[ConfigManager]   第\(lineNum + 1)行: \(key)=\(value.prefix(20))...")
            setEnvironmentVariable(key, value: value)
            loadedCount += 1
        }
        
        print("[ConfigManager] ✅ 成功加载 \(loadedCount) 个环境变量")
        
        // 验证关键变量是否设置成功
        let verifyKeys = ["OPENAI_API_KEY", "OPENAI_BASE_URL", "OPENAI_MODEL"]
        for key in verifyKeys {
            let value = ProcessInfo.processInfo.environment[key]
            print("[ConfigManager] 验证 \(key): \(value != nil ? "✅已设置" : "❌未设置")")
        }
    }
    ...
    private func setEnvironmentVariable(_ key: String, value: String) {
        setenv(key, value, 1)
        print("[ConfigManager] 🔧 设置环境变量: \(key)=\(value.prefix(20))...")
    }
    
    /// 获取OpenAI API密钥 - 优先从环境变量读取
    func getOpenAIAPIKey() throws -> String? {
        // 首次调用时加载环境变量
        if !Self.envLoaded {
            print("[ConfigManager] 🔄 初始化配置...")
            loadEnvironmentVariables()
            Self.envLoaded = true
            print("[ConfigManager] ✅ 配置初始化完成")
        }
        ...
        do {
            ...
            print("[ConfigManager] ❌ API Key未找到 - 既不在环境变量中也不在Keychain中")
            return nil
        } catch {
            print("[ConfigManager] ❌ Keychain API Key读取失败: \(error)")
            throw error
        }
    }
    ...
    /// OpenAI基础URL - 优先从环境变量读取
    var openAIBaseURL: String {
        get { 
            // 1. 优先从环境变量读取
            if let envBaseURL = ProcessInfo.processInfo.environment["OPENAI_BASE_URL"],
               !envBaseURL.isEmpty {
                return envBaseURL
            }
    ...
    /// OpenAI组织ID - 支持从环境变量读取
    var openAIOrganization: String? {
        get { 
            // 1. 优先从环境变量读取
            if let envOrgId = ProcessInfo.processInfo.environment["OPENAI_ORGANIZATION"],
               !envOrgId.isEmpty {
                return envOrgId
            }
            
            // 2. 从Defaults读取
            return Defaults[.openAIOrganization] 
        }
    ...
    var openAIModel: String {
        ...
        set { 
            ...
            selectedModelId = newValue.isEmpty ? "gpt-4o-mini" : newValue
        }
    }
    ...
    private func validateConfiguration() {
        do {
            ...
            let hasValidAPIKeyValue = apiKey != nil && !apiKey!.isEmpty && apiKey!.count >= 10
            let hasValidURL = URL(string: openAIBaseURL) != nil
            
            hasValidOpenAIKey = hasValidAPIKeyValue
            configurationValid = hasValidAPIKeyValue && hasValidURL
            isConfigured = configurationValid
            
            // 确保selectedModelId有值
            if selectedModelId.isEmpty {
                selectedModelId = openAIModel.isEmpty ? "gpt-4o-mini" : openAIModel
            }
            
            print("[ConfigManager] 配置验证结果: API Key=\(hasValidAPIKeyValue), URL=\(hasValidURL), 整体=\(configurationValid)")
        } catch {
