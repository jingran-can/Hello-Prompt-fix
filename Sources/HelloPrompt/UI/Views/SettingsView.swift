...
public struct SettingsView: View {
    ...
    private func requestSystemPermissions() async {
        ...
        if !accessibilityEnabled {
            ...
            alert.informativeText = "Hello Prompt v2需要辅助功能权限来监听全局快捷键 Ctrl+U。请在系统设置中启用此权限。"
            ...
            if response == .alertFirstButtonReturn {
                // 标记开始权限配置流程
                PermissionManager.shared.startPermissionConfigurationFlow()
                ...
                // 显示友好的后续指导
                let guidanceAlert = NSAlert()
                guidanceAlert.messageText = "权限设置指导"
                guidanceAlert.informativeText = """
                请在系统设置中完成以下步骤：
                
                1. 在"辅助功能"列表中找到 Hello Prompt
                2. 如果没有，点击"+"按钮添加
                3. 确保 Hello Prompt 的开关处于开启状态
                4. 完成后返回此应用
                
                ⚠️ 重要：权限配置完成后，请重启此应用以确保快捷键正常工作。
                """
                guidanceAlert.addButton(withTitle: "我知道了")
                
                await MainActor.run {
                    guidanceAlert.runModal()
                    // 用户确认后结束权限配置流程
                    PermissionManager.shared.endPermissionConfigurationFlow()
                }
                
                LogManager.shared.info("SettingsView", "用户已打开系统设置，建议重启应用以确保权限生效")
            }
        }
