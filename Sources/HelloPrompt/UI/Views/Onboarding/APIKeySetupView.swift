            ...
            VStack(spacing: 20) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    if !validationErrors.filter({ $0.field == .apiKey }).isEmpty {
                        let apiKeyErrors = validationErrors.filter { $0.field == .apiKey }
                        ForEach(0..<apiKeyErrors.count, id: \.self) { index in
                            let error = apiKeyErrors[index]
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    if !validationErrors.filter({ $0.field == .baseURL }).isEmpty {
                        let baseURLErrors = validationErrors.filter { $0.field == .baseURL }
                        ForEach(0..<baseURLErrors.count, id: \.self) { index in
                            let error = baseURLErrors[index]
                            Label(error.message, systemImage: "exclamationmark.triangle.fill")
                                .font(.caption)
                                .foregroundColor(.red)
                        }
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    if !validationErrors.filter({ $0.field == .organizationID }).isEmpty {
                        let orgIDErrors = validationErrors.filter { $0.field == .organizationID }
                        ForEach(0..<orgIDErrors.count, id: \.self) { index in
                            let error = orgIDErrors[index]
                            Label(error.message, systemImage: "exclamationmark.triangle.fill")
                                .font(.caption)
                                .foregroundColor(.red)
                        }
...
// 使用 ConfigurationValidator.swift 中定义的 ValidationError
// 删除这里的重复定义
